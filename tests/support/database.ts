...
export async function createIsolatedDatabase(): Promise<{
  ...
  assert.ok(
    ...
    "Configure a development DATABASE_URL before running database tests.",
  );
  const url = new URL(process.env.DATABASE_URL);
  assert.ok(
    ["postgres:", "postgresql:"].includes(url.protocol),
    "Database tests require a PostgreSQL connection URL.",
  );
  ...
  try {
    const migrations = await Promise.all(
      [
        ...
        "20260914090000_restaurant_workspace",
      ].map(async (migration) => {
        ...
        assert.doesNotMatch(
          sql,
          /(?:"?public"?|"?pg_catalog"?)\s*\./i,
          "Test migrations must not target shared schemas.",
        );
        assert.doesNotMatch(
          sql,
          /\b(?:SET\s+(?:LOCAL\s+)?search_path|CREATE\s+SCHEMA|ALTER\s+DATABASE|DROP\s+SCHEMA)\b/i,
          "Test migrations must remain in the isolated schema.",
        );
        return sql;
      }),
    );
    ...
    created = true;
    const current = await setup.query<{ schema_name: string }>(
