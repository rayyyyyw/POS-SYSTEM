...
export async function startIsolatedApp() {
  ...
  try {
    ...
    child = spawn(process.execPath, ["--import", pathToFileURL(resolve("tests/support/mail-capture.mjs")).href, resolve("node_modules/next/dist/bin/next"), "start", "-p", String(port), "--hostname", "127.0.0.1"], {
      ...
      env: { ...process.env, DATABASE_URL: fixture.url, BETTER_AUTH_URL: origin, BETTER_AUTH_SECRET: randomBytes(32).toString("hex"), RESEND_API_KEY: "disposable-test-key", EMAIL_FROM: "QA <sender@example.test>", POS_TEST_MAIL_CAPTURE: "1", NODE_ENV: "production" },
    });
