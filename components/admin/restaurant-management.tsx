...
export function RestaurantInvitations({ restaurant, invitations }: { restaurant: RestaurantDTO; invitations: InvitationDTO[] }) {
  const canInvite = restaurant.status === "PENDING" || restaurant.status === "ACTIVE";
  return <Panel title="Invitations" description="Track email submission, resend, or revoke before acceptance. Submission is not confirmed inbox delivery. Latest 50 invitations.">
    ...
    {!canInvite && <p className="mb-5 text-sm text-muted-foreground">Restore or reactivate this restaurant before sending invitations.</p>}
    {invitations.some(invitation => invitation.deliveryStatus === "FAILED" && invitation.status === "PENDING") && <p className="mb-5 text-sm text-muted-foreground">Submission unconfirmed? Retry delivery shows a safe error explanation. The onboarding@resend.dev test sender restricts real recipients to the Resend account address; inviting other owners requires a verified sending domain. The intended invitation email is never replaced with the test email.</p>}
