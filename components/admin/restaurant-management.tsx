...
export function OwnershipTransfer({ restaurant, members }: { restaurant: RestaurantDTO; members: MembershipDTO[] }) {
  ...
  return <Panel title="Transfer ownership" description="The current owner becomes a manager when another member takes ownership.">{restaurant.status === "ARCHIVED" ? <p className="text-xs leading-6 text-muted-foreground">Restore the restaurant before transferring ownership.</p> : candidates.length ? <ActionForm action={transferOwnership} submitLabel="Transfer ownership" variant="outline" confirmation="Transfer ownership to the selected member? The current owner will become a manager."><input type="hidden" name="restaurantId" value={restaurant.id} /><FormField name="userId" label="New owner" required options={candidates.map((member) => ({ value: member.userId, label: `${member.name} (${member.email})` }))} hint="The account must be active and email-verified." /></ActionForm> : <p className="text-xs leading-6 text-muted-foreground">Invite another member and wait for acceptance before transferring ownership.</p>}</Panel>;
}
